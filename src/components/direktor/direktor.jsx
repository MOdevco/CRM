import { Avatar, Box, Button, Checkbox, Heading, Spinner, Text } from '@chakra-ui/react'
...
const Direktor = ({id , name}) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}api/physical-stuff/by-category?cid=${id}` , {
      headers: {
      ...
      <Box bg={'white'} p={'30px'} rounded={'10px'} display={'flex'}   flexDirection={'column'} gap={'20px'}>
        ...
        <Box position={'sticky'} rounded={'10px'} flexDirection='column' zIndex={10} p={'10px'} top={0} bg={'white'} display={'flex'} justifyContent={'space-between'}>
          <Box display={'flex'} justifyContent={'space-between'}>
            <Text fontSize={'24px'} color={'#4D515A'}>{name}</Text>
        ...
        <Box display={'flex'} position={'fixed'} width={'80.6%'} bottom={-1}  bg={'white'}  p={'20px'} pl={'0'} alignItems={'center'} gap={'20px'} justifyContent={'flex-end'}>
